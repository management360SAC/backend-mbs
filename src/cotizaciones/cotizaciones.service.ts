...
export class CotizacionesService {
  ...
  async create(dto: CreateCotizacionDto): Promise<Cotizacion> {
    ...
    const cotRepo = await this.tds.getRepository(Cotizacion);
    const detalleRepo = await this.tds.getRepository(CotizacionDetalle);
  ...
  async generatePdf(id: number): Promise<Buffer> {
    ...
    if (cot.empresa_id) {
      try {
        const empRows = await cotRepo.query(
        ...
        if (emp) {
          ...
          if (cot.empresa_contacto_id) {
            const ctRows = await cotRepo.query(
      ...
      try {
        const result = await cotRepo.query(
  ...
  async enviar(id: number, dto: SendCotizacionDto): Promise<CotizacionEnvio> {
    const cotRepo = await this.tds.getRepository(Cotizacion);
